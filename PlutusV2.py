...
# Made by Isaac Delly changed by aritoke
# https://github.com/Isaacdelly/Plutus
# https://github.com/Aritoke9/PlutusV2
...
def public_key_to_address(public_key):
    # Pastikan public_key dalam bentuk bytes
    if isinstance(public_key, str):
        public_key = bytes.fromhex(public_key)  # Konversi dari hex string ke bytes

    # Langkah pertama: SHA-256 dari public_key
    sha256_result = hashlib.sha256(public_key).digest()

    # Langkah kedua: RIPEMD160 dari hasil SHA-256
    ripemd160 = RIPEMD160.new()
    ripemd160.update(sha256_result)
    hashed_public_key = ripemd160.digest()

    # Tambahkan prefix untuk alamat Bitcoin (0x00 untuk mainnet)
    var_encoded = b'\x00' + hashed_public_key

    # Tambahkan checksum
    checksum = hashlib.sha256(hashlib.sha256(var_encoded).digest()).digest()[:4]
    final_address = var_encoded + checksum

    # Ubah ke format base58 untuk menghasilkan alamat akhir
    return encode_base58(final_address)

def encode_base58(b):
    alphabet = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz'
    num = int.from_bytes(b, 'big')  # Konversi bytes ke integer
    result = ''
    while num > 0:
        num, remainder = divmod(num, 58)
        result = alphabet[remainder] + result

    # Menangani leading zero bytes untuk menjaga format base58
    for byte in b:
        if byte == 0:
